...
export async function CompanyAchizitii({
	id,
	slug,
	searchParams,
}: CompanyAchizitiiProps) {
	...
	let results: Awaited<ReturnType<typeof getCachedCompanyAchizitii>>;
	try {
		results = await getCachedCompanyAchizitii({ ...companyProps, page, perPage });
	} catch {
		return notFound();
	}
