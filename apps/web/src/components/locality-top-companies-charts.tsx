...
interface LocalityTopCompaniesChartsProps {
...
export function LocalityTopCompaniesCharts({ companies }: LocalityTopCompaniesChartsProps) {
  // Only show top 15 in bar chart for readability
  const barData = useMemo(() => {
    return companies.slice(0, 15).map((company) => ({
      name: company.name.length > 28 ? `${company.name.slice(0, 28)}...` : company.name,
  ...
  const pieData = useMemo(() => {
    ...
    for (const company of companies) {
      for (const typeItem of company.byType) {
