...
import { formatDate } from "@sicap/api";
...
export async function ContractAchizitii({ id }: { id: string }) {
	let contract: Awaited<ReturnType<typeof getCachedContractAchizitii>>;
	try {
		contract = await getCachedContractAchizitii(id);
	} catch {
		return notFound();
	}
	...
	const { entityId, numericFiscalNumber, entityName, city, county } =
		contractingAuthority;
			...
			<div className="grid sm:grid-cols-[25%,75%] mt-4">
				...
				<RowItem
					...
					value={
						<span
							className={
								sysDirectAcquisitionState.id === 7 ? "" : "text-red-500"
							}
						>
				...
				<RowItem
					label="Descriere:"
					value={
						<samp className="text-xs">{directAcquisitionDescription}</samp>
					}
				/>
				<RowItem
					label="Achizitii"
					value={
						<div>
							{directAcquisitionItems.map((item) => (
								...
								>
									<div className="font-semibold font-mono">
										{moneyRon(item.itemClosingPrice)}
									</div>
									<div className="mb-3">
										<div className="text-gray-400">
											Cantitate: {item.itemQuantity}
										</div>
										<div className="text-gray-400">
											Unitate masura: {item.itemMeasureUnit}
										</div>
